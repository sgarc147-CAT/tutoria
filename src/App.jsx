...
// Clau per detectar alumnes duplicats en importar. Fa servir el DNI si n'hi ha;
// si el DNI no s'ha pogut llegir bé del fitxer (columna no reconeguda, cel·la buida...),
// recorre a nom+cognoms+data de naixement en lloc de rebutjar l'alumne silenciosament.
// Si no hi ha prou dades ni per això, retorna "" i mai es considera duplicat.
function studentDedupeKey(s) {
  const dni = normalize(s.dni || "");
  if (dni) return "dni:" + dni;
  const nom = normalize(s.nom || "");
  const cognoms = normalize(s.cognoms || "");
  if (nom && cognoms) return "nc:" + nom + "|" + cognoms + "|" + normalize(s.dataNaixement || "");
  return "";
}
...
export default function App() {
  ...
  function addImportedStudents(newOnes) {
    const existingKeys = new Set(students.map(studentDedupeKey).filter(Boolean));
    const seenInBatch = new Set();
    const toAdd = [];
    newOnes.forEach((n) => {
      const key = studentDedupeKey(n);
      if (key && (existingKeys.has(key) || seenInBatch.has(key))) return;
      if (key) seenInBatch.add(key);
      const id = "s" + Math.random().toString(36).slice(2, 9);
      toAdd.push({ id, grup: activeGroup, ...n });
    });
    if (toAdd.length) {
      ...
      setSchedules((prev) => {
        const next = { ...prev };
        toAdd.forEach((st) => {
          next[st.id] = { reduccio: 0, periods: [{ id: "p1", dataInici: "", dataFinal: "", ticks: makeEmptyGrid() }] };
        });
        return next;
      });
    }
    return { added: toAdd.length, skipped: newOnes.length - toAdd.length };
  }
...
function ImportTab({ activeGroup, onImport }) {
  ...
  function confirmImport() {
    const { added, skipped } = onImport(preview);
    setResult({ added, skipped, total: preview.length });
    setPreview(null);
    setFileName("");
    ...
    <div>
      ...
      {preview && (
        <Card className="p-5 mt-6">
          ...
          <p className="text-xs text-slate-400 mt-3">Si un alumne ja existeix a la classe (mateix DNI, o mateix nom+cognoms+data de naixement si el DNI no s'ha pogut llegir), no es duplicarà.</p>
        </Card>
      )}
      ...
      {result && (
        <Card className="p-5 mt-6">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            <Badge tone="green" icon={CheckCircle2}>{result.added} alumne(s) afegit(s)</Badge>
            {result.skipped > 0 && <Badge tone="orange" icon={AlertTriangle}>{result.skipped} descartat(s) per ja existir</Badge>}
          </div>
          {result.added === 0 && result.total > 0 && (
            <p className="text-xs text-orange-500 mt-3">
              Cap alumne s'ha afegit: tots coincidien amb algú que ja hi era. Si creus que no hauria de ser així, revisa que el fitxer tingui columnes de nom, cognoms i DNI/NIE ben identificables.
            </p>
          )}
        </Card>
      )}
