...
import {
  ...
  CalendarDays, Percent, ClipboardList, ArrowLeft
} from "lucide-react";
...
function scheduleStatus(student, schedule, classGrid = makeEmptyGrid()) {
  let totalHours = 0, conflicts = 0, totalWeeks = 0;
  const perPeriod = (schedule.periods || []).map((p) => {
    let hoursPerWeek = 0, periodConflicts = 0;
    p.ticks.forEach((row, r) => row.forEach((on, c) => {
      if (on) {
        hoursPerWeek += 1;
        if (classGrid[r][c]) periodConflicts += 1;
      }
    }));
    let weeks = 0;
    if (p.dataInici && p.dataFinal) {
      const start = new Date(p.dataInici);
      const end = new Date(p.dataFinal);
      if (end > start) weeks = Math.max(1, Math.round((end - start) / (1000 * 60 * 60 * 24 * 7)));
    }
    const hours = hoursPerWeek * weeks;
    totalHours += hours;
    conflicts += periodConflicts;
    totalWeeks += weeks;
    return { id: p.id, dataInici: p.dataInici, dataFinal: p.dataFinal, hoursPerWeek, weeks, hours, conflicts: periodConflicts };
  });
  const target = TOTAL_FCT_HOURS * (1 - (schedule.reduccio || 0) / 100);
  return { perPeriod, totalHours, totalWeeks, target, complete: totalHours >= target && target > 0, conflicts };
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const payload = { activeGroup, students, companies, raWeights, moduleCourse, classTemplate, schedules };
    const t = setTimeout(() => window.__SAVE_DATA__(payload), 800);
    return () => clearTimeout(t);
  }, [activeGroup, students, companies, raWeights, moduleCourse, classTemplate, schedules]);
  ...
  const groupStudents = useMemo(() => students.filter((s) => (s.grup || "ADM2") === activeGroup), [students, activeGroup]);
  ...
  const scheduleStatuses = useMemo(() => {
    const m = {};
    students.forEach((s) => { m[s.id] = scheduleStatus(s, schedules[s.id] || { periods: [], reduccio: 0 }, classGrids[s.id]); });
    return m;
  }, [students, schedules, classGrids]);
  ...
  function addImportedStudents(newOnes) {
    setStudents((prev) => {
      const existingDnis = new Set(prev.map((s) => normalize(s.dni)));
      const merged = [...prev];
      newOnes.forEach((n) => {
        if (!existingDnis.has(normalize(n.dni))) {
          const id = "s" + Math.random().toString(36).slice(2, 9);
          merged.push({ id, grup: activeGroup, ...n });
          setSchedules((sPrev) => ({ ...sPrev, [id]: { reduccio: 0, periods: [{ id: "p1", dataInici: "", dataFinal: "", ticks: makeEmptyGrid() }] } }));
        }
      });
      return merged;
    });
  ...
  return (
    <div className="min-h-screen w-full bg-[#f8fafc] text-slate-800 flex flex-col md:flex-row">
      ...
      <main className="flex-1 p-4 md:p-8 max-w-6xl">
        {tab === "dashboard" && <Dashboard activeGroup={activeGroup} students={groupStudents} statuses={statuses} scheduleStatuses={scheduleStatuses} companies={companies} />}
        {tab === "import" && <ImportTab activeGroup={activeGroup} onImport={addImportedStudents} />}
        ...
        {tab === "students" && !selectedStudent && (
          <StudentsList activeGroup={activeGroup} students={groupStudents} statuses={statuses} scheduleStatuses={scheduleStatuses} onSelect={setSelectedStudentId} />
        )}
        {tab === "students" && selectedStudent && (
          <StudentDetail
        ...
        {tab === "companies" && (
          <CompaniesTab companies={companies} setCompanies={setCompanies} students={students} onAssignCompany={assignCompanyToStudent} />
        )}
      </main>
    </div>
  );
...
function ImportTab({ activeGroup, onImport }) {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-slate-500 mb-6">Carrega un Excel (.xlsx/.xls) o CSV amb les dades de matriculació. Els camps es reconeixen automàticament. Els alumnes importats s'afegiran al grup actiu: <span className="font-medium text-sky-600">{activeGroup}</span>.</p>
      ...
      {preview && (
        <Card className="p-5 mt-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={confirmImport} className="px-4 py-2 rounded-lg bg-green-600 text-white text-sm font-medium hover:bg-green-700 flex items-center gap-1.5">
              ...
              <tbody>
                {preview.map((r, i) => (
                  <tr key={i} className="border-b border-slate-50 text-slate-600">
                    ...
                    <td className="py-2 pr-4">{r.cognoms}</td>
                    <td className="py-2 pr-4">{r.dni}</td>
                    <td className="py-2 pr-4">{r.idalu}</td>
                    <td className="py-2 pr-4">{r.emailPersonal}</td>
                    <td className="py-2 pr-4">{r.municipi}</td>
                    <td className="py-2 pr-4">{r.estudi}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-xs text-slate-400 mt-3">Els alumnes amb un DNI/NIE ja existent a la classe no es duplicaran.</p>
        </Card>
      )}
    </div>
  );
...
function StudentsList({ activeGroup, students, statuses, scheduleStatuses, onSelect }) {
    ...
    <div>
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {filtered.map((s) => {
          ...
          const sc = scheduleStatuses[s.id];
          return (
            <button key={s.id} onClick={() => onSelect(s.id)} className="text-left">
...
function StudentDetail({ student, status, raWeights, moduleCourse, schedule, scheduleStatus, classGrid, classTemplate, companies, assignedCompanyId, onAssignCompany, onBack, onUpdateStudent, onUpdateNote, onUpdateNotaEmpresa, onUpdateSchedule, onAddPeriod, onRemovePeriod, onUpdatePeriod, onToggleTick }) {
  ...
  return (
    <div className={`-m-4 md:-m-8 p-4 md:p-8 min-h-[calc(100vh-2rem)] ${bgClass}`}>
      <button onClick={onBack} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-sky-600 mb-4">
    ...
    <div className="space-y-5">
        ...
        return (
          <Card key={p.id} className="p-5">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
              ...
              <div className="flex items-center gap-2">
                {ps && <Badge tone="sky">{ps.hoursPerWeek}h/setmana × {ps.weeks} setm. = {ps.hours}h</Badge>}
      ...
      <Card className="p-5 flex flex-wrap items-center justify-between gap-4">
        <div className="text-sm text-slate-600">
          <p>Total: <strong>{scheduleStatus.totalHours}h</strong> en {scheduleStatus.totalWeeks} setmanes repartides en {schedule.periods.length} període(s) (objectiu {scheduleStatus.target}h)</p>
...
function CompaniesTab({ companies, setCompanies, students, onAssignCompany }) {
